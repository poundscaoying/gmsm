...
func (c *sm4Cipher) generateSubkeys(keyBytes []byte) {
	fmt.Println("--------------welcome crypto sm4 generateSubkeys------------")
...
func encryptBlock(subkeys []uint32, dst, src []byte) {
	fmt.Println("--------------welcome crypto sm4 encryptBlock------------")
	cryptBlock(subkeys, dst, src, false)
}
func decryptBlock(subkeys []uint32, dst, src []byte) {
	fmt.Println("--------------welcome crypto sm4 decryptBlock------------")
	cryptBlock(subkeys, dst, src, true)
}

func cryptBlock(subkeys []uint32, dst, src []byte, decrypt bool) {
	fmt.Println("--------------welcome crypto sm4 cryptBlock------------")
	var m = make([]uint32, 4)
	var o = make([]uint32, 4)
